...
<?php
  ...
  $id=$_GET['U'];
    if (isset($_POST['send'])) {
    	$s=mysqli_query($conn, "DELETE FROM grade WHERE candidatanationalid='$id'");   
        $sg=mysqli_query($conn, "DELETE FROM candidate WHERE candidatanationalid='$id'");
		...
		<div class="col-lg-6 ml-5">
			<form class="row ml-5 sticky-top" method="POST">
					<div class="col-lg-12  sticky-top" style="
						...
						padding: 10px;
					">
						Delete Candidate <strong>with ID#<?php echo$_GET['U']?></strong>
					</div>
				...
				<?php }else{?>
					<div class='col-lg-12 alert alert-danger'><i class='fa fa-info-circle'></i> No Candidate found <a href="candidatelist.php"><strong>Back to list</strong></a></div>
				<?php }?>
